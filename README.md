...
Aplicación en vanilla JavaScript con tres vistas: Home, Chat y About. El chat usa un servidor local en Node.js para enviar mensajes a OpenRouter y devolver respuestas con personalidad de los personajes de The Simpsons.
...
Si tenías una key expuesta previamente, rota la credencial en OpenRouter y usa una nueva.
...
- `server.js`: servidor HTTP local que sirve la app y expone `/api/chat`.
...
2. El frontend envía un `POST` a `/api/chat` con:
   ...
   - `message`
   - `history`
3. El backend arma el payload para OpenRouter, lo envía y devuelve una respuesta normalizada.
...
- `AI_QUOTA_EXCEEDED` (429): revisa la cuota o el rate limit de OpenRouter.
...
- [ ] Variables de entorno cargadas en Vercel (`OPENROUTER_API_KEY`, `OPENROUTER_MODEL`, `PORT` si aplica).
